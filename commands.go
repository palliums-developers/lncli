...
func printRespJSON(resp proto.Message) {
	...
	if err != nil {
		errStr := "unable to decode response: " + err.Error()
		globalChan <- errStr
		return
	}
...
var createCommand = cli.Command{
	...
	Flags: []cli.Flag{
		...
		cli.StringFlag{
			Name:  "password",
			Usage: "the new wallet password",
		},
		cli.StringFlag{
			Name:  "mnemonic_base64",
			Usage: "Base64 encoded 24-word mnemonic separated by spaces",
		},
	},
	Action: actionDecorator(create),
}

var mnemonicWordsCommand = cli.Command{
	Name:     "mnemonicwords",
	Category: "Startup",
	Usage:    "generate mnemonic words.",
	Description: `
	The mnemonicwords command is used to generate mnemonic words for
	create command use.
	`,

	Action: actionDecorator(mnemonicwords),
}
...
func create(ctx *cli.Context) error {
	...
	if ctx.IsSet("password") {
		...
		// return an error.
		pwErr := walletunlocker.ValidatePassword(pwd)
		if pwErr != nil {
			return pwErr
		}
		pw1 = pwd
	} else {
		return fmt.Errorf("password argument missing")
	}
	...
	var mnemonic string
	if ctx.IsSet("mnemonic_base64") {
		mnemonicBase64 := ctx.String("mnemonic_base64")

		tmpMnemonic, err := base64.StdEncoding.DecodeString(string(mnemonicBase64))
		if err != nil {
			return fmt.Errorf("unable to decode mnemonic: %v", err)
		}
		mnemonic = string(tmpMnemonic)
		hasMnemonic = true
	} else {
		hasMnemonic = false
	}
	...
	resultStr := "!!!YOU MUST WRITE DOWN THIS SEED TO BE ABLE TO " +
		"RESTORE THE WALLET!!!\n" +
	...
	for i := 0; i < len(colWords); i += numCols {
		resultStr = resultStr + fmt.Sprintf("%2d. %3s  %2d. %3s  %2d. %3s  %2d. %3s\n",
			i+1, colWords[i], i+2, colWords[i+1], i+3,
			colWords[i+2], i+4, colWords[i+3])
	}

	resultStr = resultStr + "---------------END LND CIPHER SEED-----------------"
	...
	globalChan <- resultStr
	return nil
}

func mnemonicwords(ctx *cli.Context) error {
	ctxb := context.Background()
	client, cleanUp := getWalletUnlockerClient(ctx)
	defer cleanUp()
	...
	cipherSeedMnemonic = seedResp.CipherSeedMnemonic
	var words string
	for _, word := range cipherSeedMnemonic {
		if word != "" {
			word += " "
		}
		words += word
...
var changePasswordCommand = cli.Command{
	...
	Flags: []cli.Flag{
		cli.StringFlag{
			...
			Usage: "wallet current password",
		},
		cli.StringFlag{
			Name:  "new_password",
			Usage: "wallet new password",
		},
	},
...
func changePassword(ctx *cli.Context) error {
	...
	if ctx.IsSet("current_password") {
		currentPw = []byte(ctx.String("current_password"))
	} else {
		return fmt.Errorf("current_password argument missing")
	}

	if ctx.IsSet("new_password") {
		newPw = []byte(ctx.String("new_password"))
	} else {
		return fmt.Errorf("new_password argument missing")
	}
	...
	globalChan <- "change password successfully!"
	return nil
}
