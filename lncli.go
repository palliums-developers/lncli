...
func init() {
	// prevent program exits when an error is encountered
	cli.OsExiter = func(code int) {}
}

func CommandExecute(args string) string {
	app := cli.NewApp()
	app.Name = "lncli"
	...
	go runCmdRoutune(app, arguments)
...
func runCmdRoutune(app interface {
	Run(arguments []string) (err error)
}, arguments []string) {
	defer func() {
		if err := recover(); err != nil {
			globalChan <- fmt.Sprintf("[lncli] %v\n", err)
		}
	}()
	if err := app.Run(arguments); err != nil {
		// fatal(err)
		globalChan <- err.Error()
	}
}
